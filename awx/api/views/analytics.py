...
class AnalyticsGenericView(APIView):
    ...
    def _get_analytics_path(request_path):
        ...
        path_specific = parts[len(parts) - 1]
    ...
    def _send_to_analytics(self, request, method):
        try:
            ...
            if method == "GET":
                ...
                response = requests.post(url, auth=(rh_user, rh_password), verify=CERT_PATH, params=request.query_params, headers=headers, json=request.data)
            else:
